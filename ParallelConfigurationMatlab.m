function varargout = ParallelConfigurationMatlab(varargin)
% PARALLELCONFIGURATIONMATLAB MATLAB code for ParallelConfigurationMatlab.fig
%      PARALLELCONFIGURATIONMATLAB, by itself, creates a new PARALLELCONFIGURATIONMATLAB or raises the existing
%      singleton*.
%
%      H = PARALLELCONFIGURATIONMATLAB returns the handle to a new PARALLELCONFIGURATIONMATLAB or the handle to
%      the existing singleton*.
%
%      PARALLELCONFIGURATIONMATLAB('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in PARALLELCONFIGURATIONMATLAB.M with the given input arguments.
%
%      PARALLELCONFIGURATIONMATLAB('Property','Value',...) creates a new PARALLELCONFIGURATIONMATLAB or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before ParallelConfigurationMatlab_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to ParallelConfigurationMatlab_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help ParallelConfigurationMatlab

% Last Modified by GUIDE v2.5 15-Dec-2011 17:34:51
...
gui_State = struct('gui_Name',       mfilename, ...
    ...
    'gui_OpeningFcn', @ParallelConfigurationMatlab_OpeningFcn, ...
    'gui_OutputFcn',  @ParallelConfigurationMatlab_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before ParallelConfigurationMatlab is made visible.
function ParallelConfigurationMatlab_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to ParallelConfigurationMatlab (see VARARGIN)

% Choose default command line output for ParallelConfigurationMatlab
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes ParallelConfigurationMatlab wait for user response (see UIRESUME)
...
function varargout = ParallelConfigurationMatlab_OutputFcn(hObject, eventdata, handles)
